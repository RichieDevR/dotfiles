...
AWESOME_FZF_LOCATION="$HOME/.zsh/scripts/awesome-fzf.zsh"
...
fzf-config-status(){
  config rev-parse --git-dir > /dev/null 2>&1 || { echo "You are not in a git repository" && return }
    local selected
    selected=$(config -c color.status=always status --short |
        fzf --height 50% "$@" --border -m --ansi --nth 2..,.. \
